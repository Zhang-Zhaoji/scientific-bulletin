...
.search-result .article-card-actions {
  display: flex;
  align-items: center;
  gap: 8px;
  flex-shrink: 0;
}
.search-result .favorite-toggle {
  padding: 4px 8px;
  font-size: 12px;
...
.search-result .favorite-toggle.active {
  background: var(--accent, #0969da);
  color: white;
  border-color: var(--accent, #0969da);
}
.search-result .favorite-toggle img {
  width: 14px;
  height: 14px;
}
.search-sub {
...
.search-tier.tier-top {
  background: #ffe3e3;
  color: #b3261e;
}
.search-tier.tier-rec {
  background: #e9f3ff;
  color: #0f5d9e;
}
.search-tier.tier-default {
  background: #eef3f8;
  color: var(--muted);
}
.search-score {
