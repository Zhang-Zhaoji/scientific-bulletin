...
<div id="dashboard-charts" hidden>
  <div class="chart-grid">
    <div class="chart-card"><h3>国家发文量热力图</h3><div id="chart-country-heatmap" class="chart-container"></div></div>
    <div class="chart-card"><h3>评分分布</h3><div id="chart-score-dist" class="chart-container"></div></div>
    <div class="chart-card">
      <h3>研究领域分布</h3>
      <div class="chart-controls">
        <select id="radar-mode" class="date-input">
          <option value="overall">整体分布</option>
          <option value="country">国家对比</option>
        </select>
        <select id="radar-normalize" class="date-input">
          <option value="absolute">绝对数量</option>
          <option value="contribution">贡献比例</option>
          <option value="focus">国家中心归一化</option>
        </select>
    ...
    <div class="chart-card chart-wide"><h3>国家 × 领域交叉热力图</h3><div id="chart-country-field" class="chart-container"></div></div>
    <div class="chart-card"><h3>期刊发文量 TOP20</h3><div id="chart-journal-bar" class="chart-container"></div></div>
    <div class="chart-card"><h3>机构排名（发文量 × 评分）</h3><div id="chart-institution-scatter" class="chart-container"></div></div>
    <div class="chart-card">
      <h3>交叉标签词云</h3>
      <div class="chart-controls">
    ...
    <div class="chart-card"><h3>国家平均评分</h3><div id="chart-country-score" class="chart-container"></div></div>
  </div>
</div>
"""
    ...
    function renderWordcloud() {
      ...
      query(sql).then(function (res) {
        ...
        if (chart) chart.setOption({
          ...
          series: [{ type: "wordCloud", shape: "circle", sizeRange: [12, 50], rotationRange: [-30, 30],
            rotationStep: 15, gridSize: 8, data: cloudData,
            textStyle: { color: function(){return "rgb(" + Math.floor(Math.random()*100+50) + "," + Math.floor(Math.random()*100+80) + "," + Math.floor(Math.random()*100+120) + ")";} } }]
        }, true);
      });
    }
...
.trend-mode.active { background: #0868ac; color: #fff; border-color: #0868ac; }
...
@media (max-width: 1200px) { .chart-grid { grid-template-columns: repeat(2, 1fr); } .chart-wide { grid-column: span 2; } }
@media (max-width: 768px) { .chart-grid { grid-template-columns: 1fr; } .chart-wide { grid-column: span 1; } .chart-container { height: 300px; } }
</style>"""
