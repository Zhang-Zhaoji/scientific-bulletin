...
- **13个可视化模块**：国家/地区发文量热力图、国家/地区平均评分热力图、研究领域分布雷达图（支持国家/地区对比和贡献比例/专注度归一化）、国家/地区评分领域分布雷达图、机构排名散点图、交叉标签词云（7种配色方案）、国家/地区×领域交叉热力图（支持绝对数量/贡献度/专注度三种模式）、评分分布、时间趋势（支持平滑）、领域时间趋势、国家/地区发文趋势、期刊发文量TOP20、期刊评分箱线图
- **交互控件**：日期范围选择器、领域多选筛选、归一化模式切换、图表展开/收起——所有图表实时联动更新
- 每个图表卡片右上角有展开按钮（⤢），点击可占满整行查看大图
